...
int k = n - 1; // k là cột đối xứng mà biễu diễn từ 0 nên trừ đi 1
    for(int i=0; i<n-1; i++){ // i chạy tới n-2 vì phải để n-1 để in dấu * cho đáy, nếu cho i chạy từ i = 1 thì i chỉ tới i <= n-1
        for(int j=0; j<(2*n-1); j++){ // đã dùng đúng số cột, không giống như số hàng ở trên (chừa lại 1 hàng n-1)
            if (j == k-i || j == k+i) printf(" * ");
        else printf("   ");
    }
printf("\n");
}
for (int i=0; i<(2*n-1) ;i++) printf(" * "); // in * cho tất cả các cột ở hàng n-1
}
