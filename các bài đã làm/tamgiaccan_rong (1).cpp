...
{                  // tam giác cân thì dùng ma trận m*(2m-1) = m hàng và 2m-1 cột
    int k = n - 1; // k là cột đối xứng mà biễu diễn từ 0 nên trừ đi 1
    for (int i = 0; i < n - 1; i++)
    { // i chạy tới n-2 vì phải để n-1 để in dấu * cho đáy, nếu cho i chạy từ i = 1 thì i chỉ tới i <= n-1
        for (int j = 0; j < (2 * n - 1); j++)
        { // đã dùng đúng số cột, không giống như số hàng ở trên (chừa lại 1 hàng n-1)
            if (j == k - i || j == k + i)
                printf(" * ");
            else
                printf("   ");
    ...
    for (int i = 0; i < (2 * n - 1); i++)
        printf(" * "); // in * cho tất cả các cột ở hàng n-1
}
...
void TamGiacVuongCanTrai(int n)
{ // tam giac có n hàng, hàng thứ n thì sẽ có tương ứng n *
...
void TamGiacVuongCanTraiNguoc(int n)
{ // tam giac có n hàng, hàng thứ n thì sẽ có tương ứng n *, Ngược đầu thì cho chạy từ lớn về bé
...
void TamGiacVuongCanPhai(int n)
{ // tam giac có n hàng, hàng thứ n thì sẽ có tương ứng n *

    for (int i = 1; i <= n; i++)
    {
        for (int k = 0; k < n - i; k++) // hàng thứ bao nhiều thì có tương ứng số * nên số " " được in ra sẽ bằng (n - số * ở hàng đó)
        {
            printf("   ");
        }
...
void TamGiacVuongCanPhaiNguoc(int n)
{ // tam giac có n hàng, hàng thứ n thì sẽ có tương ứng n *
...
void TamGiacVuongCanRongPhai(int n)
{
    ...
    while (k <= n - 3)
    {
        ...
        for (int i = 1; i <= a; i++) // in ra (n-2-k) số 0
        {
            printf(" 0 ");
        }
        printf(" * ");
    ...
    for (int i = 1; i <= n; i++) // in * hàng cuối
    {
        printf(" * ");
    }
}
